import { Algoan, EventName, PostSubscriptionDTO, ServiceAccount } from '@algoan/rest';
...
export class AlgoanService implements OnModuleInit {
  ...
  constructor(
    ...
    private readonly serviceAccounService: AlgoanServiceAcountService,
  ) {}
  ...
  public async initRestHooks(target: string, events: EventName[] = [], secret?: string): Promise<void> {
    this.algoanClient.serviceAccounts = await this.serviceAccounService.findAll();
  ...
  public async saveServiceAccount(payload: ServiceAccountCreatedDTO): Promise<void> {
    const addedServiceAccount = await this.serviceAccounService.findById(payload.serviceAccountId);
  ...
  public async updateServiceAccount(payload: ServiceAccountUpdatedDTO): Promise<void> {
    const updatedServiceAccount = await this.serviceAccounService.findById(payload.serviceAccountId);
